...
public class ConcatenatePDFActivity extends SNPDFActivity {
  ...
  private Builder getAlertDialogForPasswordRequired(final int password) {
    return getAlertDialog().setTitle("PDF is encrypted!")
  ...
  private class Concatenate extends AsyncTask<String, Void, Boolean> {
    ...
    protected Boolean doInBackground(String... params) {
      ...
      try {
        if (password1_req) {
          reader1 = new PdfReader(firstFile.getAbsolutePath(), password1.getBytes());
        } else {
          reader1 = new PdfReader(firstFile.getAbsolutePath());
        }

        if (password2_req) {
          reader2 = new PdfReader(secondFile.getAbsolutePath(), password2.getBytes());
        } else {
        ...
        if (thirdFile != null) {
          if (password3_req) {
            reader3 = new PdfReader(thirdFile.getAbsolutePath(), password3.getBytes());
          } else {
        ...
        if (fourthFile != null) {
          if (password4_req) {
            reader4 = new PdfReader(fourthFile.getAbsolutePath(), password4.getBytes());
          } else {
        ...
        if (fifthFile != null) {
          if (password5_req) {
            reader5 = new PdfReader(fifthFile.getAbsolutePath(), password5.getBytes());
          } else {
