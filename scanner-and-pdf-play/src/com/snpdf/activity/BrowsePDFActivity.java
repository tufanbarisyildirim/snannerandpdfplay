...
import com.snpdf.layout.IFolderItemListener;
...
public class BrowsePDFActivity extends SNPDFActivity implements IFolderItemListener {
  ...
  public void OnFileClicked(File file) {
    ...
    } else if (fileType != null && SNPDFCContstants.FILE_TYPE_TXT.equals(fileType)
        && !file.getName().toLowerCase().endsWith(".txt") && !file.getName().toLowerCase().endsWith(".log")
        && !file.getName().toLowerCase().endsWith(".csv")) {
      getAlertDialog().setTitle("Invalid selection")
          .setMessage("You can only select a .txt, .log or .csv file for this conversion!")
          .setPositiveButton("OK", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
              dialog.dismiss();
            }
    ...
    } else if (fileType != null && SNPDFCContstants.FILE_TYPE_HTML.equals(fileType)
        && !file.getName().toLowerCase().endsWith(".htm") && !file.getName().toLowerCase().endsWith(".html")) {
      getAlertDialog().setTitle("Invalid selection")
          .setMessage("You can only select a .htm OR .html file for this conversion!")
          .setPositiveButton("OK", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
              dialog.dismiss();
            }
    ...
    } else if (fileType != null && SNPDFCContstants.FILE_TYPE_DOC.equals(fileType)
